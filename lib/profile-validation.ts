...
import { PROFILE_PHOTO_TOO_LARGE_MESSAGE, isProfilePhotoDataUrlTooLarge } from "@/lib/profile-photo";
...
export const PROFILE_MESSAGES = {
  ...
  phoneInvalid: "Numéro de téléphone invalide. Format attendu : +225 00 00 00 00 00.",
  fullAddressRequired: "L'adresse précise est obligatoire.",
  companyEntryDateRequired: "La date d'entrée dans l'entreprise est obligatoire.",
  companyEntryDateInvalid: "Date d'entrée invalide. Utilisez le format AAAA-MM-JJ.",
  ...
  maritalStatusInvalid: "Situation matrimoniale invalide. Choisissez Célibataire ou Marié(e).",
  childrenCountRequired: "Le nombre d'enfants est obligatoire.",
  childrenCountInvalid: "Le nombre d'enfants doit être un nombre entier positif ou 0.",
...
export function profileAddressError(value: unknown, required: boolean) {
  return required ? profileTextRequiredError(value, PROFILE_MESSAGES.fullAddressRequired) : null;
}
...
export function profileDateError(value: unknown, required: boolean) {
  ...
  return Number.isNaN(date.getTime()) ? PROFILE_MESSAGES.companyEntryDateInvalid : null;
}
...
export function profileChildrenCountError(value: unknown, required: boolean) {
  ...
  return Number.isInteger(parsed) && parsed >= 0 ? null : PROFILE_MESSAGES.childrenCountInvalid;
}
...
export function profilePhotoDataUrlError(value: unknown) {
  return isProfilePhotoDataUrlTooLarge(String(value ?? "")) ? PROFILE_PHOTO_TOO_LARGE_MESSAGE : null;
}

function addError(errors: ProfileValidationErrors, field: ProfileField, message: string | null) {
  if (message) errors[field] = message;
}

export function validateOnboardingProfileInput(input: ProfileValidationInput) {
  ...
  addError(errors, "lastName", profileTextRequiredError(input.lastName, PROFILE_MESSAGES.lastNameRequired));
  addError(errors, "gender", profileGenderError(input.gender, true));
  addError(errors, "maritalStatus", profileMaritalStatusError(input.maritalStatus, true));
  addError(errors, "childrenCount", profileChildrenCountError(input.childrenCount, true));
  ...
  addError(errors, "companyEntryDate", profileDateError(companyEntryDate, true));
  addError(errors, "cnpsNumber", profileCnpsError(input.cnpsNumber, true));
  addError(errors, "profilePhotoUrl", profilePhotoDataUrlError(input.profilePhotoUrl));
...
export function validateProfileUpdateInput(input: ProfileValidationInput) {
  ...
  addError(errors, "maritalStatus", profileMaritalStatusError(input.maritalStatus, false));
  addError(errors, "childrenCount", profileChildrenCountError(input.childrenCount, false));
  addError(errors, "profilePhotoUrl", profilePhotoDataUrlError(input.profilePhotoUrl));
